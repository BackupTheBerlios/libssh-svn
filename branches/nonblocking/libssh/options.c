...
SSH_OPTIONS *ssh_options_new(){
    ...
    option->remoteport=22; /* set the default port */
    option->fd=-1;
...
void ssh_options_set_port(SSH_OPTIONS *opt, unsigned int port){
    opt->remoteport=port&0xffff;
    opt->localport=port&0xffff;
}
SSH_OPTIONS *ssh_options_copy(SSH_OPTIONS *opt){
    ...
    ret->remotehost	=	opt->remotehost;
    ret->remoteport	= 	opt->remoteport;

	ret->localhost =	opt->localhost;
	ret->localport =	opt->localport;
...
void ssh_options_set_bind(SSH_OPTIONS *opt, u32 localhost,int localport)
{
    opt->localhost=localhost;
