        ...
        int maxpacket,BUFFER *payload)
{
    ...
    while (packet_wait(session,SSH2_MSG_CHANNEL_OPEN_CONFIRMATION)); // FIXME BLOCKING BAD HACK
...
static int channel_request(CHANNEL *channel,char *request, BUFFER *buffer,int reply)
{
	...
	while(packet_wait(session,SSH2_MSG_CHANNEL_SUCCESS) != SSH_OK);// FIXME BLOCKING BAD HACK
