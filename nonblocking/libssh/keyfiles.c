...
int ssh_is_server_known(SSH_SESSION *session){
    ...
    tokens=ssh_parse_knownhost(session->options->known_hosts_file,
        "",session->current_crypto->server_pubkey_type); // FIXME session->options->remotehost
    if(tokens==NULL)
...
int ssh_write_knownhost(SSH_SESSION *session){
    ...
    if (!strcmp(session->current_crypto->server_pubkey_type, "ssh-rsa1")) {
       ...
       snprintf(buffer, sizeof(buffer), "%s %d %s %s\n",
               "", rsa_size << 3, // session->options->remotehost FIXME common
               e_string, n_string);
#ifdef HAVE_LIBGCRYPT
       free(e_string);
       gcry_mpi_release(e);
       ...
       snprintf(buffer,sizeof(buffer),"%s %s %s\n","",session->current_crypto->server_pubkey_type,pubkey_64);// -- common FIXME session->options->host
       free(pubkey_64);
    }
